...
process *crear_Proceso(process *_pr){
	process *_p = (process *)malloc(sizeof(process)); 	//Tamaño a reservar
	//_p->id=_pr->id; 					//Identificador
	//_p->delay=_pr->delay;					//Tiempo de espera para la estructura
	//strcpy(_p->nombrePROCESO, _pr->nombreProceso);	//copiar nombre a la estructura
	//_p->nombrePROCESO;
	//_p->proceso_realizar = _pr->proceso_realizar;	//Asignacion proceso
	//_p->estado=_pr->estado;				//Aginacion 
	return _p;
}
